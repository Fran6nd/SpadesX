...
void grenade_explode_at(server_t* server, player_t* player, vector3f_t pos)
{
    ...
    for (int z = (int)pos.z - 1; z <= (int)pos.z + 1; ++z) {
             ...
             y + 1 >= 0 && y + 1 <= server->s_map.map.size_y))
        {
            ...
            vector3i_t* neigh = getGrenadeNeighbors(pos_int);
    ...
    DL_FOREACH_SAFE(player->grenade, grenade, tmp)
    {
        if (grenade->sent) {
            ...
            if ((get_nanos() - grenade->time_since_sent) / 1000000000.f >= grenade->fuse) {
                int denied = (scripting_on_grenade_explode(server, player, grenade->position) == SCRIPTING_DENY);
                if (!denied) {
                    grenade_explode_at(server, player, grenade->position);
                }
                grenade->sent = 0;
